...
export default function ColorPopper({ action, Noteid, setNotes, updatecolor }) {
    ...
    return (
        <div >
            ...
            <Popper id={id} open={open} anchorEl={anchorEl} className=' relative left-[200px]'>
                <Box className=' relative  top-[20px]' sx={{ border: 1, p: 1, bgcolor: 'background.paper', display: "flex", alignItems: "center", height: 25 }} >
                    {
                        colors.map((colr) => (<div onClick={() => selectColor(colr)} style={{ backgroundColor: colr, height: 25, width: 25, margin: 10, borderRadius: 50 }}></div>))
                    }
                </Box>
            </Popper>
        </div>
